...
class AreaController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view', 'getAreasOptions'),
				'users'=>array('*'),
			),
	...
	public function actionGetAreasOptions($pid = 0)
	{
		$pid = $_POST['pid'];
		header('Content-Type: text/html; charset=utf-8');
		$areas = Area::getAreas($pid);
		if(!$areas)
			return;
		$output = '<select name="subarea">';
		foreach($areas as $id => $v) {
			$output .= "<option value='$id'>$v</option>";
		}
		$output .= "</select>";
		echo $output;
	}
